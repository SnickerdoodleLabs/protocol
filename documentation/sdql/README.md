...
### insightPlatform (required)
A complete URL specifying the location of the Insight Platform aggregator associated with this SDQL file. 
...
- `name`: The *name* sub-keyword indicates which attribute must be accessed in the DW persistence layer. Supported attributes include:
...
- `return`: The return sub-keyword specifies the object type that will be returned by a query. Supported types include:
    ...
    - `object`: returns an object to describe the referenced attributed. 
    - `array`: returns an array to describe the referenced attributed.
    - `string`: returns a string to describe the attribute of interest. 
...
### insights (optional)
The [*insights*](/documentation/sdql/README.md#returns) keyword is used to specify one or more insight objects that may be delivered to an insight aggregator.<br>
**Each insight** object has the following sub-keywords:
- `name:` type of the insight:
...
- `target:` the target expression which must be evaluated to truthy for this insight to be returned.
- `returns:` a message or another expression. If expression, the value of the expression is returned.


### compensations (required)
The *compensations* keyword is used to declare one or more possible digital assets associated with the SDQL file. All the compensations may have some common parameters which are defined first. Every property is optional.

#### Common parameters for compensations
- `recipientAddress`: address where to send digital assets
- `productId`: an URL to the products for physical products
- `shippingAddress`: physical address to ship products.

Each compensation has two parts. The first part is metadata about the compensation and eligibility requirements. The second part is the parameters needed from the users to generate the compensation.
...
- `chainId`: Chain Id if it's a web3 reward
- `callback`: an object that encapsulate parameters to generate the reward
    - `parameters`: the user parameters specific to this reward
    - `data`: system parameters for tracking, default values, etc.
- `alternatives`: alternative compensations if this compensation is out of stock.
...
- `target`: the targeting expression. Ads are shown to a user if the expression evaluates to `True`.
