...
describe("Consent", () => {
    ...
    beforeEach(async() => {
        ...
        await consent.initialize(accounts[0].address, "www.businessuri.com");
    });
    ...
    describe("optIn" , function () {
        ...
        it("Does not allow opt-ins when function is disabled.", async function () {
            ...
            await expect(consent.optIn(1, "www.uri.com/1")).to.revertedWith("Consent: Open opt-ins are currently disabled");
        ...
        it("Does not allow opt-ins when function is paused.", async function () {
            ...
            await expect(consent.optIn(1, "www.uri.com/1")).to.revertedWith("Pausable: paused");
        ...
        it("Does not allow opt-ins with an existent token id.", async function () {
            ...
            await expect(consent.connect(accounts[2]).optIn(1, "www.uri.com/1")).to.revertedWith('ERC721: token already minted');
    ...
    describe("restrictedOptIn" , function () {
        ...
        it("Allows user who as been signed for to opt-in", async function () {
            ...
            await consent.connect(accounts[1]).restrictedOptIn(1, "www.uri.com/1", 1, sig);
        ...
        it("Does not allows user who has not been signed for to opt-in", async function () {
            ...
            // Business user signs user 1's address
            // pass in address in lowercase to match Solidity string conversion
            let sig = await getSignature(accounts[1].address.toLowerCase(), 1, "www.uri.com/1");

            // User 10 tries to call restricted opt in using signature the business entity signed for User 1
            await expect(consent.connect(accounts[10]).restrictedOptIn(1, "www.uri.com/1", 1, sig))
        ...
        it("Does not allow opt-ins when function is paused.", async function () {
            ...
            // User 1 can now call restricted opt in if business entity has signed to approve them
            await expect(consent.connect(accounts[1]).restrictedOptIn(1, "www.uri.com/1", 1, sig))
        ...
        it("Does not allow approved user to call restricted opt-ins with a different token id.", async function () {
            ...
            await expect(consent.connect(accounts[1]).restrictedOptIn(1, "www.uri.com/1", 2, sig))
                .to.revertedWith("Consent: Contract owner did not sign this message");
        });
    ...
    describe("optOut" , function () {
        ...
        it("Does not allow an address to opt-out another user's token.", async function () {
            ...
            await expect(consent.connect(accounts[2]).optOut(1)).to.revertedWith("ERC721Burnable: caller is not owner nor approved");
    ...
    describe("requestForData" , function () {
        ...
        it("Allows the MINTER_ROLE to request for data and emits RequestForData with the correct arguments", async function () {
        ...
        it("Does not allow non-MINTER_ROLE addresses to request for data", async function () {

            // call opt in with an account that did not deploy the contract
            await expect(consent.connect(accounts[1]).requestForData("cid123"))
                .to.revertedWith(`AccessControl: account ${accounts[1].address.toLowerCase()} is missing role ${minterRoleBytes}`);
    ...
    describe("safeTransferFrom" , function () {
        ...
        it("Does not allow token owner to transfer Consent token after opting-in.", async function () {
            ...
            // check token balance of the account has 1
            await expect(consent.connect(accounts[1])["safeTransferFrom(address,address,uint256)"](accounts[1].address, accounts[2].address, 1))
        ...
        it("Does not allow contract owner/any other address to transfer Consent token after opting-in.", async function () {
            ...
            // check token balance of the account has 1
            await expect(consent["safeTransferFrom(address,address,uint256)"](accounts[1].address, accounts[2].address, 1))
                .to.revertedWith("ERC721: transfer caller is not owner nor approved");

            // check token balance of the account has 1
            await expect(consent.connect(accounts[2])["safeTransferFrom(address,address,uint256)"](accounts[1].address, accounts[2].address, 1))
            .to.revertedWith("ERC721: transfer caller is not owner nor approved");
    ...
    describe("disableOpenOptIn" , function () {
        ...
        it("Does not allows address without PAUSER_ROLE to disable the open opt-in.", async function () {
            ...
            await expect(consent.connect(accounts[1]).disableOpenOptIn())
                .to.revertedWith(`AccessControl: account ${accounts[1].address.toLowerCase()} is missing role ${pauserRoleBytes}`); 
    ...
    describe("enableOpenOptIn" , function () {
        ...
        it("Does not allows address without PAUSER_ROLE to enable open opt-in.", async function () {
            ...
            await expect(consent.connect(accounts[1]).enableOpenOptIn())
                .to.revertedWith(`AccessControl: account ${accounts[1].address.toLowerCase()} is missing role ${pauserRoleBytes}`); 
    ...
    describe("setBaseURI" , function () {
        ...
        it("Allows MINTER_ROLE to update the base URI.", async function () {
        ...
        it("Does not allow address without MINTER_ROLE to update the baseURI", async function () {

            await expect(consent.connect(accounts[1]).setBaseURI("www.newURI.com"))
                .to.revertedWith(`AccessControl: account ${accounts[1].address.toLowerCase()} is missing role ${minterRoleBytes}`);
    ...
    describe("setTrustedForwarder" , function () {
        ...
        it("Allows ADMIN_ROLE to update the trusted forwarder address.", async function () {

            // set trusted forwarder address
            await consent.setTrustedForwarder('0xdD2FD4581271e230360230F9337D5c0430Bf44C0'); 

            expect(await consent.trustedForwarder()).to.eq('0xdD2FD4581271e230360230F9337D5c0430Bf44C0');
        });

        it("Does not allow address without ADMIN_ROLE to update the forwarder address.", async function () {

            
            await expect(consent.connect(accounts[1]).setTrustedForwarder('0xdD2FD4581271e230360230F9337D5c0430Bf44C0'))
                .to.revertedWith(`AccessControl: account ${accounts[1].address.toLowerCase()} is missing role ${adminRoleBytes}`);
    ...
    describe("tokenURI" , function () {
        ...
        it("Returns the correct token uri", async function () {
             ...
             // call opt in with an account that did not deploy the contract
             await consent.connect(accounts[1]).optIn(1, "/age?=1/location?=0/gender?=0");

             expect(await consent.tokenURI(1)).to.eq("www.businessuri.com/age?=1/location?=0/gender?=0");
