...
describe("WDoodleToken", () => {
    ...
    it("User deposits then withdraws, receives native tokens" , async function () {
        ...
        let contractNativeBeforeDeposit = await provider.getBalance(wDoodleToken.address);
        ...
        let contractNativeAfterDeposit = await provider.getBalance(wDoodleToken.address);

        let doodleBalanceAfterDeposit = await wDoodleToken.balanceOf(addr1.address);
        expect(Number(ethers.utils.formatEther(doodleBalanceAfterDeposit))).to.eq(1);
        
        await wDoodleToken
            .connect(addr1)
            .unwrapAndWithdraw(ethers.utils.parseEther('1'));
        
        let doodleBalanceAfterWithdrawal = await wDoodleToken.balanceOf(addr1.address);
        expect(Number(ethers.utils.formatEther(doodleBalanceAfterWithdrawal))).to.eq(0);

        let nativeTokensAfterWithdrawal = await provider.getBalance(addr1.address);
        let contractNativeAfterWithdrawal = await provider.getBalance(wDoodleToken.address);
        ...
        expect(Number(ethers.utils.formatEther(contractNativeAfterDeposit))).to.be.eq(1);
