const { consentContract, consentFactory, CC, CCFactory } = require("./constants.js");

task("checkConsentOwner", "Check consent contract owner")
    .setAction(async () => {
        ...
        const owner = await consentContractHandle.connect(accounts[0]).owner();
        console.log("owner is:", owner);
});

task("checkBalanceOf", "Check consent contract owner")
    .setAction(async () => {
        const accounts = await hre.ethers.getSigners();
        ...
        const owner = await consentContractHandle.connect(accounts[0]).balanceOf(accounts[1].address);
        console.log("Token balance is:", owner);
});

task("createConsentContract", "Calls the consent factory and deploys a Consent contract")
...
.setAction(async (taskArgs) => {
    ...
    console.log("");
    await consentFactoryContractHandle.connect(accounts[0]).createConsent(owner, "www.uri.com").then(tx => tx.wait());
    console.log("Owner of Consent deployed :", accounts[1].address);  
    
    const deployedBeaconProxyConsentAddresses = await consentFactoryContractHandle.connect(accounts[0]).getConsentBP(accounts[1].address);
    console.log("Deployed BeaconProxy Consent address", deployedBeaconProxyConsentAddresses);
    console.log("");
});

task("requestForData", "Submits a transaction to the mock consent contract to emit RequestForData event.")
    .addParam("consentaddress", "Target consent address.")
    .addParam("owneraddressindex", "Index or owner address of the connected wallet to generate Signer object to sign the tx.")
    ...
    .setAction(async (taskArgs) => {
        ...
        await consentContractHandle.connect(accounts[taskArgs.owneraddressindex]).requestForData(multihash).then(tx => tx.wait());
...
task("checkConsentsDeployedByOwner", "")
    ...
    .setAction(async (taskArgs) => {
        ...
        const logs = await consentFactoryContractHandle.filters.ConsentDeployed(taskArgs.owneraddress);
        ...
        _logs.forEach( (log, index) => {
            ...
            console.log("  Address of the deployed Consent BeaconProxy:", log.args.consentAddress)
        })
        console.log("");
});

task("listRequestedDataByOwner", "Returns a list of requested data by an address")
    .addParam("consentaddress", "Target consent address.")
    .addParam("owneraddress", "Address of data requester.")
    .setAction(async (taskArgs) => {
        ...
        // declare the filter parameters of the event of interest
        const logs = await consentContractHandle.filters.RequestForData(taskArgs.owneraddress);
...
task("grantRole", "Grant specific role on the consent contract.")
    ...
    .setAction(async (taskArgs) => {
        ...
        await consentContractHandle.connect(accounts[taskArgs.owneraddressindex]).grantRole(roleBytes, taskArgs.address);

        console.log("");
        console.log("Consent address:", taskArgs.consentaddress);
        console.log("");
        console.log("Address " + taskArgs.address + " has been granted role "+ taskArgs.role);
...
task("revokeRole", "Revokes a specific role on the consent contract.")
    ...
    .setAction(async (taskArgs) => {
        ...
        await consentContractHandle.connect(accounts[taskArgs.owneraddressindex]).revokeRole(roleBytes, taskArgs.address);

        console.log("");
        console.log("Consent address:", taskArgs.consentaddress);
        console.log("");
        console.log("Address " + taskArgs.address + " has been revoked of role "+ taskArgs.role);
