...
function evaluateReturn(userQueryResult, win, lose, response) {
  if (win != "" && lose != "") {
    return userQueryResult
      ? response[win.replace("$", "")]
      : response[lose.replace("$", "")];
  } else {
...
async function processReturn(userId, returnUrl, userReturn, queryResults, cid) {
  if (userReturn.name == "callback") {
    await sendJSON(
      { value: userReturn.message, userId: userId, cid: cid },
      returnUrl,
    );
  } else if (userReturn.name == "query_response") {
    await sendJSON(
      { value: queryResults[userReturn.query], userId: userId, cid: cid },
      returnUrl,
    );
  }
}
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      for (let j = 0; j < returnLogics.length; j++) {
        ...
        const userReturn = evaluateReturn(
          userQueryResult,
          win,
          lose,
          data.returns,
