...
const queryCid = IpfsCID("QmQBy4Q5Ya8PY8byxNgnZT35Tc3V3VhoUun5VeJ3y9HBRE");
const returns = [
  "not qualified",
  "",
  "unknown",
...
const rewardParameters = [] as IDynamicRewardParameter[];

// Parameters for /insights/responses/preview
const answeredQueries = ["some string"] as InsightString[]; // pass to deliverInsights as "insigts".
...
describe("Sign proper data for InsigtPlatform APIs", () => {
  test("insights/responses", async () => {
    ...
    const generatedDataWalletKeyResult =
      await cryptoUtils.createEthereumPrivateKey();
    const generatedDataWalletKey = generatedDataWalletKeyResult._unsafeUnwrap();

    const derivedDWAddress =
      cryptoUtils.getEthereumAccountAddressFromPrivateKey(
