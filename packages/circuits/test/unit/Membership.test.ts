...
describe("Proof of Membership", () => {
  test("Generate Proof of Membership", async () => {
    ...
    const Identities: Map<string, Identity> = new Map<Names, Identity>(
    ...
    const signalHash: Field = Poseidon.hash([...signalFields]);
    const signalHashSquared: Field = signalHash.mul(signalHash);
    ...
    const merkleRoot: Field = Identity_Tree.getRoot(); // Calculate your merkleRoot reference (this can be computed independently by anyone)

    // construct the private witness object for Bob
    const userIndex = BigInt(0);
    const userIdentity = Identities.get("Bob")!;
    const w = Identity_Tree.getWitness(userIndex); // calculate the witness for the first slot with your private data
