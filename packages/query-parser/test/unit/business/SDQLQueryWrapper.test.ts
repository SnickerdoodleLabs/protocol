...
describe("SDQLQueryWrapper with Avalanche", () => {
  test("avalanche 2 has 6 query schema", () => {
    const sdqlSchema = sdqlQueryWrapperFactory.makeWrapper(
      new SDQLQuery(cid, avalanche2Schema),
    );
    const querySchema = sdqlSchema.getQuerySchema();
    expect(Object.keys(querySchema).length).toBe(6);
  });
  ...
  //   const sdqlSchema = mocks.makeQueryWrapper(avalanche2Schema);
